...
# Fast, fixed perturbations. Keep the audit short in CI.
SCENARIOS = (
...
def run(df, ema_fast=EMA_FAST, ema_slow=EMA_SLOW,
        atr_sl=ATR_SL_MULTIPLIER, rr=REWARD_RISK, score=MIN_SCORE):
...
def scenario_audit(df):
    ...
    for name, ef, es, sl, rr, score in SCENARIOS:
        ...
        status = (
            "PASS"
            if r["expectancy_r"] > 0 and r["profit_factor"] > 1 and cost_pass
            else "FAIL"
        )
        print(
            ...
            f"cost0.10R={'PASS' if cost_pass else 'FAIL'} status={status}"
        )
        rows.append({
            "scenario": name,
...
def main():
    ...
    print(f"BUY                : trades={d['BUY'][0]} wins={d['BUY'][1]} netR={d['BUY'][2]}")
    print(f"SELL               : trades={d['SELL'][0]} wins={d['SELL'][1]} netR={d['SELL'][2]}")
    ...
    rows = []
    for label, result in (("FULL", full), ("TRAIN_60", train), ("HOLDOUT_40", hold)):
    ...
    pd.DataFrame(scenarios).to_csv("gold_parameter_stability.csv", index=False)
