...
public sealed class V14PrecisionBot : Robot
{
    ...
    [Parameter("Risk %", DefaultValue = 0.25, MinValue = 0.01, MaxValue = 2.0, Step = 0.01)]
    public double RiskPercent { get; set; }

    [Parameter("Daily Loss %", DefaultValue = 1.0, MinValue = 0.1, MaxValue = 5.0, Step = 0.1)]
    public double DailyLossPercent { get; set; }

    [Parameter("Max Consecutive Losses", DefaultValue = 3, MinValue = 1, MaxValue = 10)]
    public int MaxConsecutiveLosses { get; set; }

    [Parameter("Max Spread Pips", DefaultValue = 2.0, MinValue = 0.1, MaxValue = 20.0, Step = 0.1)]
    public double MaxSpreadPips { get; set; }

    [Parameter("Buy Min Score", DefaultValue = 78, MinValue = 70, MaxValue = 100)]
    public int BuyMinimumScore { get; set; }

    [Parameter("Sell Min Score", DefaultValue = 82, MinValue = 70, MaxValue = 100)]
    public int SellMinimumScore { get; set; }

    [Parameter("ATR Period", DefaultValue = 14, MinValue = 5, MaxValue = 50)]
    public int AtrPeriod { get; set; }

    [Parameter("ATR SL x", DefaultValue = 1.6, MinValue = 0.5, MaxValue = 5.0, Step = 0.1)]
    public double AtrStopMultiplier { get; set; }

    [Parameter("Reward/Risk", DefaultValue = 1.8, MinValue = 1.0, MaxValue = 5.0, Step = 0.1)]
    public double RewardRisk { get; set; }

    [Parameter("EMA Fast", DefaultValue = 20, MinValue = 2, MaxValue = 100)]
    public int EmaFastPeriod { get; set; }

    [Parameter("EMA Slow", DefaultValue = 50, MinValue = 5, MaxValue = 200)]
    public int EmaSlowPeriod { get; set; }

    [Parameter("Structure Lookback", DefaultValue = 20, MinValue = 5, MaxValue = 100)]
    public int StructureLookback { get; set; }

    [Parameter("ADX Period", DefaultValue = 14, MinValue = 5, MaxValue = 50)]
    public int AdxPeriod { get; set; }

    [Parameter("Volume Multiplier", DefaultValue = 1.10, MinValue = 0.5, MaxValue = 5.0, Step = 0.05)]
    ...
    protected override void OnStart()
    {
        ...
        Print("V14 started: {0} {1}", SymbolName, Bars.TimeFrame);
    }
    ...
    protected override void OnBarClosed()
    {
        ...
        if (Bars.Count < Math.Max(EmaSlowPeriod + 60, StructureLookback + 60))
            return;
        ...
        var minScore = direction == TradeType.Buy ? BuyMinimumScore : SellMinimumScore;
        ...
        if (!double.IsFinite(stopPips) || stopPips <= 0 || !double.IsFinite(takePips) || takePips <= 0)
            return;
        ...
        Print(
            "OPEN {0} score={1} entry={2} SL={3:F1}p TP={4:F1}p H4={5} H1={6} RSI={7:F1} ADX={8:F1}",
            direction,
            signal.Score,
    ...
    private (TradeType? Direction, int Score, string H4, string H1, double Rsi, double Adx) EvaluateSignal(int index)
    {
        ...
        if (!double.IsFinite(atr) || atr <= 0 || !double.IsFinite(adx))
            return (null, 0, h4, h1, rsi, adx);

        var atrMedian = AtrMedian(index, 50);
        var atrOk = atrMedian > 0
            && atr / atrMedian >= 0.75
            && atr / atrMedian <= 1.70
            && atr / close >= 0.00025
            && atr / close <= 0.02;
        ...
        var buyPullback = trendBuy
            && low <= ema20 + 0.15 * atr
            && close > ema20
            && close > Bars.ClosePrices[index - 1];
        ...
        if (sep >= 0.35)
        {
            if (trendBuy) buyScore += 5;
            if (trendSell) sellScore += 8;
        }
        ...
        {
            if (h4 != "BUY" || h1 != "BUY" || !trendBuy || !atrOk || (!buyPullback && !buyBreak) || rsi < 53)
                side = null;
        }
        else if (side == TradeType.Sell)
        {
            if (h4 != "SELL" || h1 != "SELL" || !trendSell || !atrOk || !sellBreak || rsi > 46 || adx < 25 || diMinus - diPlus < 5)
                side = null;
        }

        return (side, score, h4, h1, rsi, adx);
    }
    ...
    private static string GetTrend(Bars bars, ExponentialMovingAverage fast, ExponentialMovingAverage slow)
    {
        ...
        return fast.Result.LastValue > slow.Result.LastValue
            ? "BUY"
            : fast.Result.LastValue < slow.Result.LastValue
                ? "SELL"
                : "HOLD";
    }
    ...
    private double AtrMedian(int index, int lookback)
    {
        ...
        var values = Enumerable.Range(start, index - start)
            .Select(i => _atr.Result[i])
    ...
    {
        var dailyLoss = _dayStartBalance <= 0
            ? 0
            : Math.Max(0, (_dayStartBalance - Account.Equity) / _dayStartBalance * 100.0);

        var openPositions = Positions.FindAll(Label, SymbolName).Length;
        return dailyLoss < DailyLossPercent
    ...
    private void ValidateParameters()
    {
        ...
        if (DailyLossPercent <= 0 || DailyLossPercent > 5)
            throw new ArgumentOutOfRangeException(nameof(DailyLossPercent));
        if (MaxConsecutiveLosses < 1)
            throw new ArgumentOutOfRangeException(nameof(MaxConsecutiveLosses));
        if (MaxSpreadPips <= 0)
            throw new ArgumentOutOfRangeException(nameof(MaxSpreadPips));
        if (BuyMinimumScore < 70 || SellMinimumScore < 70)
            throw new ArgumentOutOfRangeException(nameof(BuyMinimumScore));
        if (AtrPeriod < 5 || AtrStopMultiplier <= 0 || RewardRisk < 1)
            throw new ArgumentOutOfRangeException(nameof(AtrPeriod));
        if (EmaFastPeriod >= EmaSlowPeriod)
            throw new ArgumentException("EMA Fast must be below EMA Slow.");
        if (StructureLookback < 5 || AdxPeriod < 5 || VolumeMultiplier <= 0)
            throw new ArgumentOutOfRangeException(nameof(StructureLookback));
    }
}
